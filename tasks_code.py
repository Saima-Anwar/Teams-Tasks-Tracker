# tasks_code.py
# Team Task Tracker App (Streamlit) - Final Full Version
...
def load_data(path: str = DATA_FILE) -> pd.DataFrame:
    if os.path.exists(path):
        for enc in ("utf-8","utf-8-sig","cp1252","latin1"):
            try: return normalize(pd.read_csv(path, encoding=enc))
            except Exception: continue
    return empty_df()

def save_data(df: pd.DataFrame, path: str = DATA_FILE):
...
DATE_COLS = ["Assigned Date","ETA","Date of Completion"]
...
if use_date: d1s,d2s = pd.to_datetime(str(d1)), pd.to_datetime(str(d2))
...
with tab_dash:
    ...
    if filtered.empty: st.info("No data in current filter.")
    else:
...
with tab_entry:
    ...
    with st.form("task_form", clear_on_submit=not keep_values):
        ...
        with col1:
            cat_choice = st.selectbox("Category", ["(Add New…)"]+list(all_categories), index=1 if all_categories else 0)
            category = st.text_input("New Category", value="") if cat_choice=="(Add New…)" else cat_choice
            task_name = st.text_input("Task Name")
            per_choice = st.selectbox("Person", ["(Add New…)"]+list(all_people), index=1 if all_people else 0)
            person = st.text_input("New Person", value="") if per_choice=="(Add New…)" else per_choice
            stat_choice = st.selectbox("Status", ["(Add New…)"]+list(all_statuses), index=1 if all_statuses else 0)
            status = st.text_input("New Status", value="") if stat_choice=="(Add New…)" else stat_choice
        with col2:
        ...
        if st.form_submit_button("Save Task"):
            changed=False
            if category and not _contains_ci(masters["Category"],category): masters["Category"].append(category); changed=True
            if person and not _contains_ci(masters["Person"],person): masters["Person"].append(person); changed=True
            if status and not _contains_ci(masters["Status"],status): masters["Status"].append(status); changed=True
            if changed: save_masters(masters); st.session_state["masters"]=load_masters()
            def norm_date(d): return str(d) if d else ""
            new_row = {
                "Category":category.strip(),"Task Name":task_name.strip(),"Person":person.strip(),"Status":status.strip(),
                "Assigned Date":norm_date(assigned_date),"ETA":norm_date(eta),
                "Date of Completion":norm_date(completion_date),"Notes/Issues":(notes or "").strip(),
            }
            st.session_state["tasks"]=pd.concat([st.session_state["tasks"],pd.DataFrame([new_row])],ignore_index=True)
            save_data(st.session_state["tasks"]); st.success("Task added and saved.")
...
with tab_manage:
    ...
    edited=st.data_editor(show.assign(Delete=False),column_config=cfg,hide_index=True,use_container_width=True,
                          num_rows="dynamic",key="editor_grid")
    ...
    with col1:
        ...
        if st.button("Save Category"):
            if cat_new.strip() and not _contains_ci(masters["Category"],cat_new):
                masters["Category"].append(cat_new.strip()); save_masters(masters)
                st.session_state["masters"]=load_masters(); st.success("Category added"); st.rerun()
    ...
    with col2:
        ...
        if st.button("Save Person"):
            if per_new.strip() and not _contains_ci(masters["Person"],per_new):
                masters["Person"].append(per_new.strip()); save_masters(masters)
                st.session_state["masters"]=load_masters(); st.success("Person added"); st.rerun()
    ...
    with col3:
        ...
        if st.button("Save Status"):
            if stat_new.strip() and not _contains_ci(masters["Status"],stat_new):
                masters["Status"].append(stat_new.strip()); save_masters(masters)
                st.session_state["masters"]=load_masters(); st.success("Status added"); st.rerun()
